...
output_path = "./output/aggressive_output_vehicle_count.mp4"
...
frame_count = 0
total_detected_vehicles = set()  # Track all vehicles ever detected

# Very aggressive parameters for maximum detection
confidence_threshold = 0.2  # Very low threshold to catch all vehicles
min_vehicle_area = 200  # Very low area threshold

print("Starting AGGRESSIVE vehicle counting with video visualization...")
...
while cap.isOpened():
    ...
    frame_count += 1
    try:
        # Run YOLOv8 detection with very low confidence threshold
        results = model(frame, classes=VEHICLE_CLASSES, conf=confidence_threshold)[0]
        
        # Convert results to Supervision detections
        detections = sv.Detections.from_ultralytics(results)
        
        # Filter detections by size (very permissive)
        if len(detections) > 0:
            areas = []
        ...
        # Track all detected vehicles
        if detections.tracker_id is not None:
            for tracker_id in detections.tracker_id:
                if tracker_id is not None:
                    total_detected_vehicles.add(tracker_id)
        
        # Aggressive line crossing detection
        for i, tracker_id in enumerate(detections.tracker_id):
            if tracker_id is not None:
                ...
                if tracker_id in vehicle_positions:
                    ...
                    # Crossing from top to bottom (IN) - very lenient
                    ...
                    # Crossing from bottom to top (OUT) - very lenient
        ...
        # Calculate net vehicle count
        vehicle_count = in_count - out_count
        
        # Print periodic updates to console
        if frame_count % 30 == 0:  # Every 30 frames
            print(f"Frame {frame_count}: IN={in_count}, OUT={out_count}, NET={vehicle_count}, Currently Tracked={len([id for id in detections.tracker_id if id is not None])}, Total Ever Detected={len(total_detected_vehicles)}")
        ...
        cv2.putText(annotated_frame, f"Total Ever Detected: {len(total_detected_vehicles)}", (50, 200), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        cv2.putText(annotated_frame, f"Frame: {frame_count}", (50, 250), 
        ...
        # Display frame
        cv2.imshow("Aggressive Vehicle Counting", annotated_frame)
...
print(f"Total vehicles ever detected: {len(total_detected_vehicles)}")
